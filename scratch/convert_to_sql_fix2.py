...
try:
    with open(file_in, 'r', encoding='utf-8', errors='replace') as f:
        ...
        for row in reader:
            ...
            # Skip completely empty rows (which often appear at the end of CSVs)
            if not profile_id:
                continue
            ...
            sql = f"INSERT INTO public.transactions ({', '.join(columns)}) VALUES ({', '.join(values)});"
...
try:
    with open(file_out, 'w', encoding='utf-8') as f:
        ...
        f.write("\n".join(sql_statements))
        f.write("\n")
    print(f"Successfully generated {len(sql_statements)} SQL INSERT statements into public.transactions in {file_out}")
except Exception as e:
