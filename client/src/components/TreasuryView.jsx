...
import SpawnMonsterForm from './treasury/SpawnMonsterForm';
const TreasuryView = ({ onBack, treasuryData, profile, onRefresh, initialShowLedger = false, initialLedgerMode = 'menu' }) => {
  const [showLedger, setShowLedger] = useState(initialShowLedger);
  const [ledgerMode, setLedgerMode] = useState(initialLedgerMode);
    ...
    <div className="absolute inset-0 z-[100] bg-black animate-in fade-in duration-500 overflow-hidden">
      ...
      {showLedger && (
        <div className="absolute inset-0 bg-black/50 backdrop-blur-md z-[110] flex items-center justify-center p-4 animate-in fade-in zoom-in-95 duration-300">
           {/* Parchment Container */}
           <div className={`relative w-full ${ledgerMode === 'menu' ? 'max-w-sm aspect-[4/5]' : 'max-w-4xl h-[85%]' } overflow-hidden rounded-lg shadow-2xl flex flex-col items-center transition-all duration-500`}>
              ...
              {/* Content Container */}
              <div className="relative z-10 w-full h-full p-8 pt-12 flex flex-col items-center overflow-hidden">
                {ledgerMode === 'menu' ? (
                  <>
                    <h2 
                    ...
                    <div className="w-32 h-0.5 bg-[#4b2c20]/20 mb-4" />

                    <div className="w-full space-y-1.5 px-4">
                      <button 
                        onClick={() => setLedgerMode('new')}
                        className="w-full py-2.5 px-6 bg-black/5 hover:bg-black/10 border-2 border-[#4b2c20]/20 rounded-xl transition-all hover:scale-[1.02] active:scale-95 flex items-center justify-center gap-3 group"
                      >
                        <span className="title-font text-[#4b2c20] text-sm font-black uppercase tracking-widest">New Record</span>
                      </button>

                      <button 
                        onClick={() => setLedgerMode('spawn')}
                        className="w-full py-2.5 px-6 bg-black/5 hover:bg-black/10 border-2 border-[#4b2c20]/20 rounded-xl transition-all hover:scale-[1.02] active:scale-95 flex items-center justify-center gap-3 group"
                      >
                        <span className="title-font text-[#4b2c20] text-sm font-black uppercase tracking-widest text-red-800">Spawn New Monster</span>
                      </button>
                      ...
                      <button 
                        ...
                        className="w-full py-2.5 px-6 bg-black/5 hover:bg-black/10 border-2 border-[#4b2c20]/20 rounded-xl transition-all hover:scale-[1.02] active:scale-95 flex items-center justify-center gap-3 group"
                      >
                        <span className="title-font text-[#4b2c20] text-sm font-black uppercase tracking-widest">View Records</span>
                      </button>

                      <button 
                        disabled
                        className="w-full py-2.5 px-6 bg-black/5 border-2 border-[#4b2c20]/10 rounded-xl flex items-center justify-center gap-3 opacity-40 cursor-not-allowed"
                      >
                        <span className="title-font text-[#4b2c20]/60 text-sm font-black uppercase tracking-widest">Manage Treasure (Inactive)</span>
                      </button>
                      ...
                      <button 
                        ...
                        className="w-full py-2.5 px-6 bg-black/5 hover:bg-black/10 border-2 border-[#4b2c20]/20 rounded-xl transition-all hover:scale-[1.02] active:scale-95 flex items-center justify-center gap-3 group"
                      >
                        <span className="title-font text-[#4b2c20] text-sm font-black uppercase tracking-widest">Import Archive</span>
                      </button>
                    </div>
                    <button 
                      onClick={() => {
                      ...
                      className="mt-4 flex items-center gap-2 title-font text-[#4b2c20] font-bold uppercase text-[10px] tracking-widest hover:scale-110 transition-transform bg-black/5 px-4 py-2 rounded-full border border-[#4b2c20]/10"
                    >
                      <X size={12} />
                      Close Ledger
                    </button>
                  ...
                  <div className="w-full h-full flex flex-col">
                    <div className="flex justify-between items-center mb-6 border-b border-[#4b2c20]/10 pb-4">
                      <button 
                        ...
                        className="flex items-center gap-2 title-font text-[#2d1e1e] font-black uppercase text-sm tracking-widest hover:scale-110 transition-transform"
                      >
                        <ArrowLeft size={20} />
                        Back to Menu
                      </button>
                      <h2 
                        className="title-font text-2xl font-black uppercase tracking-widest text-[#4b2c20]"
                      >
                        {ledgerMode === 'new' ? 'New Entry' : 
                         ledgerMode === 'spawn' ? 'Summon Monster' :
                         ledgerMode === 'view' ? 'Royal Records' : 
                         ledgerMode === 'manage' ? 'Manage Treasure' : 
                         'Archive Import'}
                      </h2>
                      <button 
                        onClick={() => {
                        ...
                        className="flex items-center gap-2 title-font text-red-800 font-black uppercase text-sm tracking-widest hover:scale-110 transition-transform"
                      >
                        <X size={20} />
                        Exit
                      </button>
                    ...
                    <div className="flex-1 overflow-y-auto custom-scrollbar pr-2">
                      ...
                      {ledgerMode === 'spawn' && <SpawnMonsterForm userId={profile?.id} onSuccess={() => { setLedgerMode('manage'); handleRefresh(); }} />}
                      {ledgerMode === 'view' && <RecordsTable />}
                      {ledgerMode === 'manage' && <AccountManager userId={profile?.id} profile={profile} onBack={() => setLedgerMode('menu')} onRefresh={handleRefresh} />}
