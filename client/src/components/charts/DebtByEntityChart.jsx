  ...
  return (
    <div className="bg-[#faf4e5]/60 border border-[#8b4513]/25 rounded-xl p-2.5 shadow-sm flex flex-col h-full">
      <div className="flex justify-center items-center border-b border-[#8b4513]/20 pb-1 mb-2 flex-shrink-0 text-center">
        <h3 className="title-font text-[10px] font-black text-[#4b2c20] uppercase tracking-wider">
          {t('chart_debt_by_entity', 'Debt by Entity')}
        </h3>
      </div>
 
      <div className="space-y-1.5 overflow-y-auto pr-1 custom-scrollbar-subtle flex-grow mt-1">
        {debtByEntity.length > 0 ? (
          debtByEntity.map((ent) => {
            const pctWidth = (ent.amount / maxVal) * 100;
            return (
              <div key={ent.name} className="space-y-0.5">
                ...
                <div className="w-full bg-[#faf4e5]/80 h-1.5 rounded-full overflow-hidden border border-[#8b4513]/10">
                  <div 
